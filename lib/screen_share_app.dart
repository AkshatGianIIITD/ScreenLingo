...
class _ScreenShareAppState extends State<ScreenShareApp> {
  ...
  List<Rect> _previousBoundingBoxes = [];
  ...
  Future<List<Map<String, dynamic>>> extractTextFromFrame(
    ...
    // Mask out previous bounding boxes
    for (Rect box in _previousBoundingBoxes) {
      img.fillRect(
        image!,
        x1: box.left.toInt(),
        y1: box.top.toInt(),
        //y1: (box.top - 20).toInt(), // account for Y offset
        x2: box.right.toInt(),
        y2: (box.bottom + 10).toInt(),
        //y2: (box.bottom - 20 + 10).toInt(), // adjust height & Y offset
    ...
    for (TextBlock block in recognizedText.blocks) {
      ...
      // Save current box for next-frame masking
      _previousBoundingBoxes.add(rect);
      ...
      textData.add({
        ...
        "backgroundColor": Color(bgColor).value,
  ...
  void sendOverlayData(List<Map<String, dynamic>> textData) async {
    ...
    // double yOffset = -(statusBarHeight * devicePixelRatio);
